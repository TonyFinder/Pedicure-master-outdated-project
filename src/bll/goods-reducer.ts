let initialState = {
    ...
    priceList: [{
        ...
        price: 2300
    }, {item: 'Обработка пальчиков или стопы  без покрытия', price: 1800},],
    priceInclude: ['снятие любого покрытия','тампонирование (при необходимости)','покрытие лечебным лаком',],
}

export const goodsReducer = (state: InitialStateGoodsType = initialState, action: ActionTypes): InitialStateGoodsType => {
    switch (action.type) {
        case 'SET-CATEGORY':
            return state
        default:
...
// actions
export const setCategoryAC = (category: string) => ({type: 'SET-CATEGORY', category})

// types
type InitialStateGoodsType = typeof initialState
type ActionTypes = ReturnType<typeof setCategoryAC>
